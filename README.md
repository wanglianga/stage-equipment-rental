...
- **演出经纪**：提交演出项目（日期、排练时段、舞台规格、设备清单），推进项目状态，申请临时加场
- **剧院技术部**：确认装台窗口、供电条件、吊挂点位、场馆限制
- **设备供应商**：登记设备（灯光/音响/控台/线材/支架/运输箱）及押金条款

核心业务流程：档期申请 → 设备锁定 → 出库点验 → 装台联调 → 演后归还 → 损耗确认 → 费用结算

### 临时加场功能

支持演出方临时追加场次，系统自动执行四项检查：
1. **设备占用检查**：检查原设备是否被后续项目占用
2. **剧院窗口检查**：检查剧院是否还有装台窗口
3. **技术人员检查**：检查供应商是否能补派技术人员
4. **押金补交检查**：计算是否需要补交押金

加场通过后自动顺延：排练时段、出库点验、装台联调、归还时间

部分设备批准时：演出方确认替代方案 → 供应商备货清单同步更新
